...
class Cart
{
	...
	//новые методы для работы удаленных продуктов
    ...
    public function getRemovedProducts() {
		...
		$cart_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "cart WHERE is_deleted = 1 AND api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "' AND customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "'");
		...
		foreach ($cart_query->rows as $cart) {
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				$product_data[] = array(
					...
					'shipping'        => $product_query->row['shipping'],
					'image'           => $product_query->row['image'],
					'is_deleted'           => $product_query->row['image'],
					'option'          => $option_data,
					'download'        => $download_data,
					'quantity'        => $cart['quantity'],
					'minimum'         => $product_query->row['minimum'],
	...
	//считаю количество товаров из корзины и добавленых, иначе вылетает ошибка, что корзина пустая и выводит темплейт пустой корзины
